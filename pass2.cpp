...
bool readIntermediateFile(ifstream &readFile, bool &isComment, int &lineNumber, int &address, int &blockNumber, string &label, string &opcode, string &operand, string &comment)
{
  ...
  lineNumber = string_to_int(readTillTab(fileLine, index));
  ...
  address = str_hex_to_int(readTillTab(fileLine, index));
  ...
  {
    blockNumber = string_to_int(tempBuffer);
  }
...
string createObjectCodeFormat34()
{
  ...
  { // Immediate
    ...
    { // Error handling for Immediate with index based
      ...
      objcode = int_to_hex(str_hex_to_int(OPTAB[getRealOpcode(opcode)].opcode) + 1, 2);
    ...
    { // Immediate integer value
      ...
      {
        immediateValue = string_to_int(tempOperand);
      }
      else
      {
        immediateValue = str_hex_to_int(SYMTAB[tempOperand].address);
      }
      ...
      else
      {
        ...
        objcode += int_to_hex(immediateValue, halfBytes);
      }
      return objcode;
    }
    else if (SYMTAB[tempOperand].exists == 'n' || CSECT_TAB[currentSectName].EXTREF_TAB[tempOperand].exists == 'y')
    {
      ...
      if (SYMTAB[tempOperand].exists == 'y' && CSECT_TAB[currentSectName].EXTREF_TAB[tempOperand].exists == 'y')
      {
        ...
        modificationRecord += "M^" + int_to_hex(address + 1, 6) + '^';
    ...
    {
      int operandAddress = str_hex_to_int(SYMTAB[tempOperand].address) + str_hex_to_int(BLOCKS[BLocksNumToName[SYMTAB[tempOperand].blockNumber]].startAddress);
      ...
      { /*If format 4*/
        objcode = int_to_hex(str_hex_to_int(OPTAB[getRealOpcode(opcode)].opcode) + 1, 2);
        objcode += '1';
        objcode += int_to_hex(operandAddress, halfBytes);
      ...
      /*Handle format 3*/
      program_counter = address + str_hex_to_int(BLOCKS[BLocksNumToName[blockNumber]].startAddress);
      ...
      if (relativeAddress >= (-2048) && relativeAddress <= 2047)
      {
        objcode = int_to_hex(str_hex_to_int(OPTAB[getRealOpcode(opcode)].opcode) + 1, 2);
        objcode += '2';
        objcode += int_to_hex(relativeAddress, halfBytes);
        return objcode;
      }
      ...
      if (operandAddress <= 4095)
      {
        ...
        /*add modifacation record here*/
        modificationRecord += "M^" + int_to_hex(address + 1 + str_hex_to_int(BLOCKS[BLocksNumToName[blockNumber]].startAddress), 6) + '^';
        modificationRecord += (halfBytes == 5) ? "05" : "03";
        modificationRecord += '\n';
  ...
  else if (getFlagFormat(operand) == '@')
  {
    ...
    { // Error handling for Indirect with index based
      ...
      /*****/ } /*****/
      writeToFile(errorFile, writeData);
      objcode = int_to_hex(str_hex_to_int(OPTAB[getRealOpcode(opcode)].opcode) + 2, 2);
      objcode += (halfBytes == 5) ? "100000" : "0000";
      return objcode;
      }
      /*
    ...
    writeData = "Line: " + to_string(lineNumber);
    writeData += "Can't fit into program counter based or base register based addressing. Format 4";
    writeToFile(errorFile, writeData);
    objcode = int_to_hex(str_hex_to_int(OPTAB[getRealOpcode(opcode)].opcode) + 2, 2);
    objcode += (halfBytes == 5) ? "100000" : "0000";
  ...
  { // Literals
    ...
    if (tempOperand == "*")
    {
      tempOperand = "X'" + int_to_hex(address, 6) + "'";
      /*Add modification record for value created by operand `*` */
      modificationRecord += "M^" + int_to_hex(str_hex_to_int(LITTAB[tempOperand].address) + str_hex_to_int(BLOCKS[BLocksNumToName[LITTAB[tempOperand].blockNumber]].startAddress), 6) + '^';
      modificationRecord += int_to_hex(6, 2);
      modificationRecord += '\n';
    }
    ...
    if (LITTAB[tempOperand].exists == 'n')
    {
      ...
      objcode = int_to_hex(str_hex_to_int(OPTAB[getRealOpcode(opcode)].opcode) + 3, 2);
      objcode += (halfBytes == 5) ? "000" : "0";
      objcode += "000";
      return objcode;
    }

    int operandAddress = str_hex_to_int(LITTAB[tempOperand].address) + str_hex_to_int(BLOCKS[BLocksNumToName[LITTAB[tempOperand].blockNumber]].startAddress);
    program_counter = address + str_hex_to_int(BLOCKS[BLocksNumToName[blockNumber]].startAddress);
    program_counter += (halfBytes == 5) ? 4 : 3;
  ...
  { /*Handle direct addressing*/
    ...
    if (SYMTAB[tempOperand].exists == 'n' /*****/ || CSECT_TAB[currentSectName].EXTREF_TAB[tempOperand].exists == 'y' /*****/)
    {
      ...
      writeToFile(errorFile, writeData);
      objcode = int_to_hex(str_hex_to_int(OPTAB[getRealOpcode(opcode)].opcode) + 3, 2);
      objcode += (halfBytes == 5) ? (int_to_hex(xbpe + 1, 1) + "00") : int_to_hex(xbpe, 1);
      objcode += "000";
      return objcode;
      }
...
void writeTextRecord(bool lastRecord = false)
{
  if (lastRecord)
  {
    ...
    { // Write last text record
      writeData = int_to_hex(currentRecord.length() / 2, 2) + '^' + currentRecord;
  ...
  if (objectCode != "")
  {
    if (currentRecord.length() == 0)
    {
      writeData = "T^" + int_to_hex(address + str_hex_to_int(BLOCKS[BLocksNumToName[blockNumber]].startAddress), 6) + '^';
...
void writeEndRecord(bool write = true)
{
  ...
  { // If no operand of END
    endRecord = "E^" + int_to_hex(startAddress, 6);
  }
  ...
  { // Make operand on end firstExecutableAddress
    ...
    firstExecutableAddress = str_hex_to_int(SYMTAB[firstExecutable_Sec].address);

    endRecord = "E^" + int_to_hex(firstExecutableAddress, 6) + "\n";
  }
}
...
{
  string tempBuffer;
  intermediateFile.open("intermediate_" + file_Name); // begin
  if (!intermediateFile)
  {
  ...
  if (!ListingFile)
  {
    cout << "Unable to open file: listing_" << file_Name << endl;
    exit(1);
  }
  ...
  if (opcode == "START")
  {
    ...
    writeData = to_string(lineNumber) + "\t" + int_to_hex(address) + "\t" + to_string(blockNumber) + "\t" + label + "\t" + opcode + "\t" + operand + "\t" + objectCode + "\t" + comment;
    writeToFile(ListingFile, writeData);
  }
  else
  {
  ...
  if (BLOCKS.size() > 1)
  {
    program_section_length = length_Program;
  }
  else
  {
  ...
  writeData = "H^" + expand_str(label, 6, ' ', true) + '^' + int_to_hex(address, 6) + '^' + int_to_hex(program_section_length, 6);
  writeToFile(objectFile, writeData);
  ...
  while (opcode != "END")
  {
    while (opcode != "END" && opcode != "CSECT")
    {
      ...
      if (OPTAB[getRealOpcode(opcode)].exists == 'y')
      {
        ...
        else if (OPTAB[getRealOpcode(opcode)].format == 2)
        {
          ...
          { // If not two operand i.e. a
            ...
            {
              objectCode = OPTAB[getRealOpcode(opcode)].opcode + int_to_hex(string_to_int(operand1), 1) + '0';
            }
          ...
          { // Two operands i.e. a,b
            ...
            {
              objectCode = OPTAB[getRealOpcode(opcode)].opcode + REGTAB[operand1].num + int_to_hex(string_to_int(operand2), 1);
            }
      ...
      else if (opcode == "BYTE")
      {
        ...
        {
          objectCode = str_to_hexstr(operand.substr(2, operand.length() - 3));
        }
      }
      else if (label == "*")
      {
        if (opcode[1] == 'C')
        {
          objectCode = str_to_hexstr(opcode.substr(3, opcode.length() - 4));
        }
        else if (opcode[1] == 'X')
        {
      ...
      {
        objectCode = int_to_hex(string_to_int(operand), 6);
      }
      else if (opcode == "BASE")
      {
        if (SYMTAB[operand].exists == 'y')
        {
          base_register_value = str_hex_to_int(SYMTAB[operand].address) + str_hex_to_int(BLOCKS[BLocksNumToName[SYMTAB[operand].blockNumber]].startAddress);
          nobase = false;
        }
        else
        {
      ...
      {
        writeData = to_string(lineNumber) + "\t" + int_to_hex(address) + "\t" + " " + "\t" + label + "\t" + opcode + "\t" + operand + "\t" + objectCode + "\t" + comment;
      }
      else if (address == -1)
      {
        if (opcode == "EXTDEF")
    ...
    {
      writeData = to_string(lineNumber) + "\t" + int_to_hex(address) + "\t" + to_string(blockNumber) + "\t" + label + "\t" + opcode + "\t" + " " + "\t" + objectCode + "\t" + " ";
    }
    else if (!isComment)
    {
      writeData = to_string(lineNumber) + "\t" + int_to_hex(address) + "\t" + " " + "\t" + label + "\t" + opcode + "\t" + operand + "\t" + "" + "\t" + comment;
    }
    else
    {
      writeData = to_string(lineNumber) + "\t" + comment;
    ...
    if (!isComment && opcode != "END")
    {
      ...
      writeData = "\nH^" + expand_str(label, 6, ' ', true) + '^' + int_to_hex(address, 6) + '^' + int_to_hex(CSECT_TAB[label].length, 6);
      writeToFile(objectFile, writeData);
    }
    //********************************
    readIntermediateFile(intermediateFile, isComment, lineNumber, address, blockNumber, label, opcode, operand, comment); // Read next line
...
int main()
{
  ...
  printtab.open("tables_" + file_Name);
  writeToFile(printtab, "**********************************SYMBOL TABLE*****************************\n");
  for (auto const &it : SYMTAB)
  {
    writestring += it.first + ":-\t" + "name:" + it.second.name + "\t|" + "address:" + it.second.address + "\t|" + "relative:" + int_to_hex(it.second.relative) + " \n";
  }
