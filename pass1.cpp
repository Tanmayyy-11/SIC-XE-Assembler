...
void handle_LTORG(string &litPrefix, int &lineNumberDelta, int lineNumber, int &LOCCTR, int &lastDeltaLOCCTR, int currentBlockNumber)
{
  ...
  for (auto const &it : LITTAB)
  {
    ...
    else
    {
      lineNumber += 5;
      lineNumberDelta += 5;
      LITTAB[it.first].address = int_to_hex(LOCCTR);
      LITTAB[it.first].blockNumber = currentBlockNumber;
      litPrefix += "\n" + to_string(lineNumber) + "\t" + int_to_hex(LOCCTR) + "\t" + to_string(currentBlockNumber) + "\t" + "*" + "\t" + "=" + litValue + "\t" + " " + "\t" + " ";
...
void evaluateExpression(string expression, bool &relative, string &tempOperand, int lineNumber, ofstream &errorFile, bool &error_flag)
{
  ...
  for (int i = 0; i < expression.length();)
  {
    ...
    { // Check operand existence
      ...
      valueTemp = to_string(str_hex_to_int(SYMTAB[singleOperand].address));
    }
    ...
    { // Check expressions legallity
      writeData = "Line: " + to_string(lineNumber) + " : Illegal expression detected";
      writeToFile(errorFile, writeData);
      error_flag = true;
      Illegal = true;
      break;
    ...
    {
      writeData = "Line: " + to_string(lineNumber) + " : Illegal operator detected in expression. Found " + singleOperator;
      writeToFile(errorFile, writeData);
      error_flag = true;
      Illegal = true;
      break;
  ...
  if (!Illegal)
  {
    if (pairCount == 1)
    {
      ...
      tempOperand = int_to_hex(tempOBJ.getResult());
    }
    else if (pairCount == 0)
    {
      /*absolute*/
    ...
    {
      writeData = "Line: " + to_string(lineNumber) + " :Found Illegal expression ";
      writeToFile(errorFile, writeData);
      error_flag = true;
      tempOperand = "00000";
      relative = 0;
...
void pass1()
{
  ...
  if (!intermediateFile)
  {
    cout << "ERROR: Unable to open file: intermediate_" << file_Name << endl;
    exit(1);
  }
  ...
  if (!errorFile)
  {
    cout << "ERROR: Unable to open file: error_" << file_Name << endl;
    exit(1);
  }
  ...
  if (opcode == "START")
  {
    ...
    startAddress = str_hex_to_int(operand);
    // cout<<startAddress<<endl;
    // exit(0);
    LOCCTR = startAddress;
    writeData = to_string(lineNumber) + "\t" + int_to_hex(LOCCTR - lastDeltaLOCCTR) + "\t" + to_string(currentBlockNumber) + "\t" + label + "\t" + opcode + "\t" + operand + "\t" + comment;
  ...
  while (opcode != "END")
  {
    ...
    while (opcode != "END" && opcode != "CSECT")
    {
      ...
      if (!checkCommentLine(fileLine))
      {
        ...
        { // Label found
          if (SYMTAB[label].exists == 'n')
          {
            ...
            SYMTAB[label].address = int_to_hex(LOCCTR);
        ...
        { // Search for opcode in OPTAB
          if (OPTAB[getRealOpcode(opcode)].format == 3)
          {
            ...
            if (getFlagFormat(operand) == '=')
            {
              ...
              if (tempOperand == "*")
              {
                tempOperand = "X'" + int_to_hex(LOCCTR - lastDeltaLOCCTR, 6) + "'";
              }
        ...
        else if (opcode == "ORG")
        {
          ...
          else
          {
            ...
            if (!error_flag)
            {
              LOCCTR = str_hex_to_int(tempOperand);
            }
        ...
        else if (opcode == "EQU")
        {
          ...
          if (operand == "*")
          {
            tempOperand = int_to_hex(LOCCTR - lastDeltaLOCCTR, 6);
            relative = 1;
          }
          else if (if_all_num(operand))
          {
            tempOperand = int_to_hex(string_to_int(operand), 6);
            relative = 0;
          }
          else
          {
          ...
          SYMTAB[label].blockNumber = currentBlockNumber;
          lastDeltaLOCCTR = LOCCTR - str_hex_to_int(tempOperand);
        }
        else
        {
          readFirstNonWhiteSpace(fileLine, index, statusCode, operand);
        ...
        {
          writeData = writeDataPrefix + to_string(lineNumber) + "\t" + int_to_hex(LOCCTR - lastDeltaLOCCTR) + "\t" + " " + "\t" + label + "\t" + opcode + "\t" + operand + "\t" + comment + writeDataSuffix;
        }
        //*********************************************************************************
        else if (opcode == "EXTDEF" || opcode == "EXTREF")
        {
          writeData = writeDataPrefix + to_string(lineNumber) + "\t" + " " + "\t" + " " + "\t" + " " + "\t" + opcode + "\t" + operand + "\t" + comment + writeDataSuffix;
       /* writeData = writeDataPrefix + to_string(lineNumber) + "\t" + int_to_hex(LOCCTR-lastDeltaLOCCTR) + "\t" + blockNumberstr + "\t" + label + "\t" + opcode + "\t" + operand + "\t" + comment + writeDataSuffix;   
      }*/ }
       else if (opcode == "CSECT")
       {
         writeData = writeDataPrefix + to_string(lineNumber) + "\t" + int_to_hex(LOCCTR - lastDeltaLOCCTR) + "\t" + " " + "\t" + label + "\t" + opcode + "\t" + " " + "\t" + " " + writeDataSuffix;
       }
       //*******************************************************************************
       else
       {
         writeData = writeDataPrefix + to_string(lineNumber) + "\t" + int_to_hex(LOCCTR - lastDeltaLOCCTR) + "\t" + to_string(currentBlockNumber) + "\t" + label + "\t" + opcode + "\t" + operand + "\t" + comment + writeDataSuffix;
       }
       writeDataPrefix = "";
       writeDataSuffix = "";
      }
      ...
      BLOCKS[currentBlockName].LOCCTR = int_to_hex(LOCCTR); // Update LOCCTR of block after every instruction
      getline(sourceFile, fileLine);                            // Read next line
      string k="";
    ...
    if (opcode != "END")
    {
      ...
      CSECT_TAB[currentSectName].LOCCTR = int_to_hex(LOCCTR - lastDeltaLOCCTR, 6);
      ...
      writeToFile(intermediateFile, writeDataPrefix + to_string(lineNumber) + "\t" + int_to_hex(LOCCTR - lastDeltaLOCCTR) + "\t" + to_string(currentBlockNumber) + "\t" + label + "\t" + opcode);
  ...
  LOCCTR = str_hex_to_int(BLOCKS[currentBlockName].LOCCTR);

  handle_LTORG(writeDataSuffix, lineNumberDelta, lineNumber, LOCCTR, lastDeltaLOCCTR, currentBlockNumber);

  writeData = to_string(lineNumber) + "\t" + int_to_hex(LOCCTR - lastDeltaLOCCTR) + "\t" + " " + "\t" + label + "\t" + opcode + "\t" + operand + "\t" + comment + writeDataSuffix;
  writeToFile(intermediateFile, writeData);
  ...
  {
    LocctrArr[it.second.number] = str_hex_to_int(it.second.LOCCTR);
  ...
  for (auto const &it : BLOCKS)
  {
    ...
    {
      BLOCKS[it.first].startAddress = int_to_hex(LocctrArr[it.second.number - 1]);
    }
  }

  length_Program = LocctrArr[totalBlocks - 1] - startAddress;
